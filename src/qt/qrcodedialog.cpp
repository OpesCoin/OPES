...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("OPES")
    updateDisplayUnit();
}
...
{
    QString ret = QString("OPES://%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non OPES unit input in lnReqAmount, we generate the URI with OPES as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::OPES, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
